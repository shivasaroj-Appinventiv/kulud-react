...
export const useUserListHelper = () => {
  ...
  const {usersList,status,totalDocs}=useAppSelector((state:RootState)=>state.userManagement);
  return {handlePageOptionsChanged,usersList,pageOptions,status,totalDocs,navigate}
};
